...
Technology stack:
Laravel 13.21, Filament 5.7.3 (Shield 4.2.0), Livewire, Pest, Tailwind 4, PHP ≥ 8.4.1 (required by `vendor/`). The Laravel Boost block above lists older versions (Laravel 11 / Filament 4 / PHP 8.3); it is regenerated by `boost:install`, so trust these versions instead.
